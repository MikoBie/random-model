...
## Read data
random_df = pd.read_csv(DATA/'study2_random_df.csv')

## Prepare data for algorithmic complexity computation
df = random_df.reset_index().pivot_table( index = ['id', 'age', 'sex', 'condition'], columns = ['ids'] )
...
seq_cmx = [ list( item[i:i+s] for i in range(0, len(item), s) if len(item[i:i+s]) > 4 ) for item in seq ]
...
data.to_csv(DATA/'study2_random_cmx.csv')
