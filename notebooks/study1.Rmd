...
cdf <- read.csv(file.path(DATA, 'compare_df.csv')) %>%
...
rdf_wide <- rdf_wide %>%
  select(id, age, sex, condition_order, condition_visible, file_name, cmx, cmx_mean, cmx_sd, cmx_rn7, correctness) %>%
...
## Wide format data
rdf_wide <- rdf_wide %>%
  select(id, age, sex, condition_order, condition_visible, file_name, cmx, cmx_mean, cmx_sd, correctness) 
...
figure1 %>%
  ggsave(filename = file.path(FIG,"figure_1.png"),
       device = "png",
       dpi = 300,
       height = 4.5,
       width = 6)
...
figure2 %>%
  ggsave(filename = file.path(FIG,"figure_2.png"),
       device = "png",
       dpi = 300,
       height = 4.5,
       width = 6)
